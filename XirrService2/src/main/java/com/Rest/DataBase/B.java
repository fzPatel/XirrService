...
import com.xirr.XirrController.MessageData;
import com.xirr.XirrController.SMSBody;
	@Path("/Ml_Master")
public class B 
	{
	...
	public String  genderData(@QueryParam("Master") String Master) 
	{
		...
		else if(Master.equalsIgnoreCase("loanDetailGrid")) {
			System.out.println(" inside loanDetailGrid");
			LdGridAr gd=new LdGridAr();
			gd=ml.loanDetailGrid();
		 response=gson.toJson(gd);
		}else if(Master.equalsIgnoreCase("maritalMstr")) {
		...
		else if(Master.equalsIgnoreCase("religionMstr")) {
		...
		else if(Master.equalsIgnoreCase("stateEkyc")) {
		...
		else if(Master.equalsIgnoreCase("tradeMstr")) {
			System.out.println(" inside stateEkyc");
			TradeDataAr gd=new TradeDataAr();
			gd=ml.tradeData();
			response=gson.toJson(gd);
		 }
		else if(Master.equalsIgnoreCase("motherTongueMstr")) {
		...
		else if(Master.equalsIgnoreCase("loanMstr")) {
		...
		else if(Master.equalsIgnoreCase("casteMstr")) {
		...
		else if(Master.equalsIgnoreCase("hrmsVoterRationMstr")) {
		...
		else if(Master.equalsIgnoreCase("hrmsmcModeMstr")) 
		{
			System.out.println(" inside hrmsvotrationData");
	...
	*/		
		...
		try {
				...
				requestUrl = fetchUrl.getSendSmsUrl() + "?username="
						  ...
						  "&text=" + URLEncoder.encode(fetchUrl.getUserText()+" "+otp, "UTF-8")+ "&dlr-mask=19" +
						  "&dlr-url";
